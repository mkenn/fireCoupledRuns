...
# MCK: so this is to create setup for RHESSys IO,
# to perform a parameter tuning for understory development
...
# Processing options
parameter_method <- "lhc" #? what's this? Latin hypercube sampling for SA?


# RHESSys Inputs
input_rhessys <- list()
#input_rhessys$rhessys_version <- "bin/rhessys5.20.1" 
input_rhessys$rhessys_version <- "../rhessys_git/rhessys/rhessys5.20.1" 
# so this is the path to the RHESSys executable, which should then also contain the WMFire library
# assuming relative paths from main fire_effects_model
# then: "../rhessys_git/rhessys/rhessys5.20.1
...
# so here I'm guessing I want to make firespread commandline -f 30 I think (?)
# input_rhessys$command_options <- c("-b -g -c -p -tchange 0 0")
...
# and here I'm assuming, to run WMFire:
# input_hdr_list$fire_def 

# Define path to a pre-selected df containing parameter sets
input_preexisting_table <- NULL

n_sim = 250 # for parameters or MC?

# Def file parameter changes
# List of lists containing def_file, parameter and parameters values
#input_def_list <- NULL
# seems to be in the format of: default file, parameter name, c(paramMin,paramMax,nsim)
# so nsim is probably the number of parameter values
# this is the parameter tuning, not the model SA
input_def_list <- list(
  # Patch parameters
  list(input_hdr_list$soil_def, "soil_depth", c(3.0, 5.0, n_sim)),
# MCK: where's canopy fraction?
# looks like it's in 0.0_misc_setup_scripts.R
# not sure where that is used
# MCK: where did you get these ranges?
  # -----
